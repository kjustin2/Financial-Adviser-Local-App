...
def validate_age_range(
    birth_date: date, min_age: int = 0, max_age: int = 150
...
def validate_date_range(
    start_date: date, end_date: date, max_range_days: int = None
) -> Tuple[bool, Optional[str]]:
    """
...
class BusinessRuleValidator:
    ...
    def validate_portfolio_allocation(allocations: dict) -> List[str]:
        ...
        if abs(total_allocation - 100) > 0.01:  # Allow for small rounding errors
            errors.append(
                f"Portfolio allocation must total 100%, currently {total_allocation}%"
            )
