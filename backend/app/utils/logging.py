...
def log_query_performance(query: str, duration: float, result_count: int = 0):
    ...
    if duration > 0.1:  # Log slow queries (>100ms)
        ...
        logger.debug(
            f"Query ({duration:.3f}s, {result_count} results): {query[:100]}..."
        )
...
def log_security_event(
    ...
    logger.warning(
        f"Security Event: {event_type} | {user_info} | {ip_info} | {details}"
    )
...
# Initialize logging
app_logger = setup_logging(level="DEBUG" if settings.debug else "INFO")
