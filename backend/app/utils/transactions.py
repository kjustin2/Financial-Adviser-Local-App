...
def safe_db_operation(
    ...
    try:
        ...
        start_time = (
            logger.performance.__enter__() if hasattr(logger, "performance") else None
        )
    ...
    def execute_with_rollback(self, operations: list):
        ...
        try:
            for i, operation in enumerate(operations):
                logger.debug(
                    f"Executing operation {i+1}/{len(operations)}: {operation.__name__}"
                )
                operation()
            ...
            logger.error(
                f"Operation failed at step {len(completed_operations)+1}: {str(e)}"
            )
            self.db.rollback()
            ...
            for i in reversed(completed_operations):
                if i < len(self.rollback_actions) and self.rollback_actions[i]:
                    try:
                        ...
                        logger.error(
                            f"Rollback action failed for operation {i}: {rollback_error}"
                        )
