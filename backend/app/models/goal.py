...
class GoalContribution(BaseModel):
    ...
    # Goal relationship
    goal_id = Column(
        Integer, ForeignKey("financial_goals.id"), nullable=False, index=True
    )
    ...
    contribution_type = Column(
        SQLEnum(ContributionType), default=ContributionType.MANUAL
    )
    notes = Column(Text, nullable=True)
    ...
    def __repr__(self):
        return (
            f"<GoalContribution(amount=${self.amount}, date={self.contribution_date})>"
        )
