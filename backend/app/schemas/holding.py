...
from ..models.holding import SecurityType, AssetClass

class HoldingBase(BaseModel):
    """Base holding schema with common fields."""
    symbol: str = Field(min_length=1, max_length=20, description="Security symbol (e.g., AAPL, VTI)")
    security_name: Optional[str] = Field(None, max_length=255, description="Full security name")
    security_type: SecurityType
    quantity: Decimal = Field(gt=0, decimal_places=6, description="Number of shares/units")
    cost_basis: Decimal = Field(gt=0, decimal_places=4, description="Cost per share/unit")
...
class HoldingResponse(BaseSchema):
    ...
    day_change_percent: Optional[Decimal] = Field(None, description="Daily percentage change")
...
class HoldingPerformance(BaseModel):
    ...
    weight_in_portfolio: Decimal = Field(description="Weight as percentage of portfolio")
